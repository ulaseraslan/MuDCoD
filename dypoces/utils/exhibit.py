...
def read_simulation_results_to_df(simul_result_path):
    """
    Read mean(ARI) of each MuSPCES, PisCES, static to pandas DataFrame.
    """
    result_dirs = [f.path for f in os.scandir(simul_result_path) if f.is_dir()]
    ...
    for i, path in enumerate(sorted(result_dirs)):
        ...
        if not simul_name.startswith("."):
            ...
            for i, mpath in enumerate(sorted(muspces_results_path)):
                ...
                result_dict["row"].append((i, case, rs, rt, Ns, Th, "muspces"))
            ...
            for i, ppath in enumerate(sorted(pisces_results_path)):
                ...
                result_dict["row"].append((i, case, rs, rt, Ns, Th, "pisces"))
                result_dict["val"].append(np.mean(temp))
            ...
            for i, spath in enumerate(sorted(static_results_path)):
                ...
                result_dict["row"].append((i, case, rs, rt, Ns, Th, "static"))
                result_dict["val"].append(np.mean(temp))

    index_row = pd.MultiIndex.from_tuples(
        result_dict["row"],
        names=["id", "case", "r_time", "r_sbj", "num-sbj", "time-horizon", "method"],
    )
...
if __name__ == "__main__":
    print(read_simulation_results_to_df("../results/simulation/"))
